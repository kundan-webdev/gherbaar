...
import { uploadTenantDocuments, uploadTenantPhoto } from '../middleware/upload.js';
...
function handlePhotoUpload(req, res, next) {
  uploadTenantPhoto.single('photo')(req, res, (err) => {
    if (err) return next(ApiError.badRequest(err.message));
    next();
  });
}
...
router.post('/:id/photo', requireRole('landlord', 'admin'), handlePhotoUpload, controller.uploadPhoto);
router.delete('/:id/photo', requireRole('landlord', 'admin'), controller.removePhoto);
